...
# Shell Shock - Console Plugin

A **Shell Shock** plugin to generate the `console` built-in module with a custom theme.
...
```bash
pnpm add -D @shell-shock/plugin-console
```
...
```bash
npm install -D @shell-shock/plugin-console
```
...
```bash
yarn add -D @shell-shock/plugin-console
```
...
Run `nx build plugin-console` to build the library.

## Running unit tests

Run `nx test plugin-console` to execute the unit tests via [Jest](https://jestjs.io).
