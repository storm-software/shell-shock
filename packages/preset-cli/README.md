...
# Shell Shock - Preset Command-line Interface
...
```bash
pnpm add -D @shell-shock/preset-cli
```
...
```bash
npm install -D @shell-shock/preset-cli
```
...
```bash
yarn add -D @shell-shock/preset-cli
```
...
Run `nx test preset-cli` to execute the unit tests via [Jest](https://jestjs.io).
