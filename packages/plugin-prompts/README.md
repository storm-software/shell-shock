...
# Shell Shock - Prompts Plugin

A **Shell Shock** plugin to generate the `prompts` built-in module with a custom theme.
...
```bash
pnpm add -D @shell-shock/plugin-prompts
```
...
```bash
npm install -D @shell-shock/plugin-prompts
```
...
```bash
yarn add -D @shell-shock/plugin-prompts
```
...
Run `nx build plugin-prompts` to build the library.

## Running unit tests

Run `nx test plugin-prompts` to execute the unit tests via [Jest](https://jestjs.io).
