...
# Shell Shock - Skills Plugin

A **Shell Shock** plugin that adds the skills command which allows users to view and manage their project related skills.
...
```bash
pnpm add -D @shell-shock/plugin-skills
```
...
```bash
npm install -D @shell-shock/plugin-skills
```
...
```bash
yarn add -D @shell-shock/plugin-skills
```
...
Run `nx build plugin-skills` to build the library.

## Running unit tests

Run `nx test plugin-skills` to execute the unit tests via [Jest](https://jestjs.io).
