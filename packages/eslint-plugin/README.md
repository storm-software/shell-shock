...
```bash
pnpm add -D @shell-shock/eslint-plugin
```
...
```bash
npm install -D @shell-shock/eslint-plugin
```
...
```bash
yarn add -D @shell-shock/eslint-plugin
```
...
The following sections outline the commands run locally to develop changes to the package.
