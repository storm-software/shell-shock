...
# Shell Shock - MCP Plugin

A **Shell Shock** plugin that adds a `mcp` command that generates an MCP stdio server exposing your CLI commands as MCP tools.
...
```bash
pnpm add -D @shell-shock/plugin-mcp
```
...
```bash
npm install -D @shell-shock/plugin-mcp
```
...
```bash
yarn add -D @shell-shock/plugin-mcp
```
...
Run `nx test plugin-mcp` to execute the unit tests via [Jest](https://jestjs.io).
