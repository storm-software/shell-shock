...
```bash
pnpm add -D @shell-shock/plugin-completions
```
...
```bash
npm install -D @shell-shock/plugin-completions
```
...
```bash
yarn add -D @shell-shock/plugin-completions
```
...
Run `nx build plugin-completions` to build the library.

## Running unit tests

Run `nx test plugin-completions` to execute the unit tests via [Jest](https://jestjs.io).
